...
async function checkMarketplaceEvents() {
  ...
  try {
    // 查询 NFTListed 事件
    console.log('📋 查询 NFTListed 事件...');
    const listedFilter = marketplace.filters.NFTListed();
    const listedEvents = await marketplace.queryFilter(listedFilter, startBlock, currentBlock);
    ...
    listedEvents.forEach((event, index) => {
      ...
      console.log('  价格:', ethers.formatEther(event.args?.price || 0), 'OKB');
      console.log('');
    });
    ...
    const delistedEvents = await marketplace.queryFilter(delistedFilter, startBlock, currentBlock);
    ...
    const soldEvents = await marketplace.queryFilter(soldFilter, startBlock, currentBlock);
