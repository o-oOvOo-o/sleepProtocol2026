...
export const timedelta = (seconds: number) => {
...
export const toGwei = (value: bigint) => {
...
export const estimatedStakeRewardXEN = (data: any) => {
  const amount = Number(data.amount ?? 0n) / 1e18;
  if (data.maturityTs != 0 && UTC_TIME > data.maturityTs) {
    const rate = (data.apy * data.term * 1_000_000) / DAYS_IN_YEAR;
    const totalReward = (Number(data.amount) * rate) / 100_000_000 / 1e18;
